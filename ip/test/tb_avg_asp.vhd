...
-- Self-checking unit testbench for avg_asp (no NoC; drives recv,
-- observes send).
--
-- Method: an independent naive reference keeps the last L raw
-- samples, sums them and applies an arithmetic >>log2(L). Matching
-- it against the DUT validates the running-sum optimisation, the
-- WARMUP gating and the window indexing against the plain
-- moving-average definition. The reference uses an arithmetic shift
-- (floor toward -inf), exactly mirroring the DUT, so it is correct
-- for negative sums as well as non-negative ones.
--
-- Coverage:
--   L=4/8/16  ramp on ch0: WARMUP then running averages.
--   Bypass    every sample forwarded unchanged.
--   Dual-ch   same ramp on both channels; independent windows/sums.
--   V1        bipolar sequence - the real ADC signal is bipolar, so
--             the negative-sum floor-rounding path is exercised, not
--             just non-negative ramps.
...
BEGIN
	...
	stimulus : PROCESS
		...
		-- Send a Conf-DP and check the DUT does not emit that cycle.
	...
	-- Drive one Data-Audio sample, verify the output against the independent reference.
	...
	BEGIN
		...
		ELSE
			...
			ELSE
				...
				END LOOP;
			END IF;
			-- arithmetic shift (floor), mirrors the DUT for negative s
...
BEGIN
	...
	-- each keeps its own window/sum so both must match.
	do_conf("0111", "0001");
	...
	-- V1: bipolar input, the real ADC signal is bipolar,
	-- so the negative-sum floor path is exercised.
